...
if (undefined === window.on) {
    Object.defineProperty(Node.prototype, "rm", {
        get() {
            return () => {
...
function gram() {
    ...
    function getWaitTime() {
        ...
        if (wpt?.loadEventEnd && wpt?.navigationStart) {
            return (wpt?.loadEventEnd - wpt?.navigationStart);
    ...
    function ajax(method, url, onLoad, onTimeout, body) {
        ...
        req.open(method, url);
        req.timeout = getWaitTime() * 2;
        req.addEventListener("load", onLoad);
        req.addEventListener("timeout", onTimeout);
        req.send(body);
    }
    function isLongRequest(e) {
        const total = e?.serverTiming.find(function findTotal(t) {
            return "total" === t.name
        })?.duration;
        return 300 < total || (e?.encodedBodySize + 5000 < e?.duration);
    }
    try {
        new PerformanceObserver(function detectSlowness(list) {
    ...
    function enhanceLinks(query, n = document) {
        function preloadLink(e) {
            ...
            if ($0("html.noPreload") || a.classList.contains("noPreload") || document.location.origin !== u.origin || a.getAttribute("download") || "_blank" === a.getAttribute("target")) {
                ...
                ajax("GET", u.href, function cachePage(r) {
                        ...
                        if (!a.classList.contains("noPreload") && cacheHeaders && isHtml) {
                            ...
                            if ($0("base").href === $0("base", doc).href && !revalidate && validFor) {
                                if (isCached(r.target.responseURL)) {
                                    ...
                                    const entry = {
                                        html: r.target.response,
                                        expires: Date.now() + (validFor * 1000),
                    ...
                    $("html,a").forEach(function disableCache(l) {
                        l.addClass("noPreload");
                    });
                });
            }
            if (isCached(u)) {
        ...
        function useCache(e) {
            ...
            if (isCached(u)) {
                ...
                $(".removable").forEach(function rm(e) {
                    e.rm();
                });
        ...
        function preloadThese(links) {
            if (!$0("html.noPreload") && links.length && document.body.contains(links.at(0))) {
                preloadLink({
                    currentTarget: links.shift()
                });
                setTimeout(preloadThese, getWaitTime() * 5, links);
            }
        }
        ...
        if (query instanceof Array) {
            ...
            $(query, n).on('mouseenter', preloadLink, {
                once: true
            })
    ...
    function enhancePicture(query, n = document) {
        ...
        function showZoom(e) {
            ...
            document.body.addClass("locked").on('keydown', closeZoom, {
                once: true
            }).appendChild(pic).on('click', closeZoom);
        }
        ...
        if (query instanceof Element && query.matches("html:not(.noPreload) .indexPage a.withFigure")) {
            try {
        ...
        function searchDown(e) {
            ...
            if (selected) {
                ...
                case 13: // enter
                    box.value = selected.innerText;
                    e.preventDefault();
                    $0("a", selected).dispatchEvent(new MouseEvent("click", {
                            cancelable: true
                        }));
                    return;
                default:
    ...
    function enhanceLazyload(query, n = document) {
        function eagerLoad(imgs) { //check that all not-lazy images are loaded before switching a lazy one to eager
            if (!$0("html.noPreload") && $("img:not([loading='lazy'])").every(function isComplete(i) {
                    return i.complete;
                })) {
                while (imgs.length) {
                    const img = imgs.pop();
        ...
        setTimeout(eagerLoad, getWaitTime() * 10, $(query, n).map(function getLazyImgs(node) {
    ...
    function init() {
        ...
        if (isLongRequest(window.performance?.getEntriesByType('navigation').pop()) || (navigator.connection?.saveData)) {
            ...
            enhanceLinks($("html:not(.noPreload) .indexPage a.withFigure"));
        }
    }
